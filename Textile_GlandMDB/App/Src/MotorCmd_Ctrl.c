...
void MotorCmd_Ctrl ( RegDef_PosCtrlCmd *PosCtrlCmd, S_MOTOR_CMD *sMotorCmd )
{
    ...
    static RegDef_PosCtrlCmd Rx_SteerCtrlCmd = { 0, 0, 0, 0 };
    ...
    switch ( sMotorCmd->eCmdMode )
    {
        ...
        case CmdMode_PosCtrl:
        {
            ...
            {
                if ( ( Rx_SteerCtrlCmd.u16Mode == CmdMode_Idle ) || ( Rx_SteerCtrlCmd.u16Mode > CmdMode_RC ) || ( Rx_SteerCtrlCmd.u16Mode == CmdMode_SpdCtrl ) )
                {
                    sMotorCmd->eCmdMode = CmdMode_FreeStop;
                }
                else
                {
                    ...
                    {
                        sMotorCmd->iqTargetPos = _IQsat ( Rx_SteerCtrlCmd.u16Ref, ParamHandle_Steer.Reg_PosMax, ParamHandle_Steer.Reg_PosMin );
                        if ( g_sMotor_Info.iqPosAbs > sMotorCmd->iqTargetPos )  //抬起
                        {
                            sTeles_Ctrl.PosCmd_Dir = CtrlDir_Foward;
                            sMotorCmd->eRunState = CmdRuning;
                            sMotorCmd->iqRuningTime = 0;
                        }
                        else if ( g_sMotor_Info.iqPosAbs < sMotorCmd->iqTargetPos )
                        {
        ...
        case CmdMode_TorqueCtrl:
        {
            //刷新目标速度
            if ( ( u16SteerCmdLastSN != Rx_SteerCtrlCmd.u16SN ) || ( eLastCmdMode != CmdMode_TorqueCtrl ) )
            {
                if ( ( Rx_SteerCtrlCmd.u16Mode == CmdMode_Idle ) || ( Rx_SteerCtrlCmd.u16Mode > CmdMode_RC ) || ( Rx_SteerCtrlCmd.u16Mode == CmdMode_SpdCtrl ) )
                {
                    sMotorCmd->eCmdMode = CmdMode_FreeStop;
                ...
                else
                {
                    ...
                    if ( sMotorCmd->eCmdMode == CmdMode_TorqueCtrl )
                    {
                        sTeles_Ctrl.PosCmd_Dir = CtrlDir_Reverse;
                        sMotorCmd->iqTargetTorque = _IQsat ( Rx_SteerCtrlCmd.u16Ref, ParamHandle_Steer.Reg_TorqueMax, ParamHandle_Steer.Reg_TorqueMin );
                        sMotorCmd->eRunState = CmdRuning;
                    }
                }
                u16SteerCmdLastSN = Rx_SteerCtrlCmd.u16SN;
            }
            eLastCmdMode = CmdMode_TorqueCtrl;
        }
        break;
        ...
        case CmdMode_FreeStop:
        {
            ...
            else if ( ( Rx_SteerCtrlCmd.u16Mode == CmdMode_Idle ) || ( Rx_SteerCtrlCmd.u16Mode > CmdMode_RC ) || ( Rx_SteerCtrlCmd.u16Mode == CmdMode_SpdCtrl ) )
            {
                sMotorCmd->eRunState = CmdStop_Invalid;
            }
            else
