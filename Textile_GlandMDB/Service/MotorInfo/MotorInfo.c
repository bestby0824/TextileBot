...
void MotorInfo_Torque ( S_MOTOR_INFO *psInfo )
{
    static uint16_t u16Cnt_Torque = 0;
    
    if ( g_sMotorCmd.eCmdMode == CmdMode_TorqueCtrl )
    {
        if ( psInfo->iqEffectCurrent > _IQdiv ( ParamHandle_Steer.Reg_CurrentMax * g_sMotorCmd.iqTargetTorque / 125, 100 ) )  //0.8倍给定
        {
            if (u16Cnt_Torque<300) u16Cnt_Torque++;
            else
            {
                 psInfo->u16Torque_State = 1;
            }
        }
        else
        {
            if ( u16Cnt_Torque > 0 ) u16Cnt_Torque--;
            else
            {
                psInfo->u16Torque_State = 0;
            }
        }
    }
    else
