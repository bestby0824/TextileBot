...
static void IntegralClear ( _iq PosUi, _iq SpdUi, _iq SpdRef );
...
//        psInfo->iqEffectCurrent = _IQdiv ( ( abs ( psInfo->iqCurrentA ) + abs ( psInfo->iqCurrentB ) + abs ( psInfo->iqCurrentC ) ), _IQ ( 1.732 ) );
        ...
        if ( ++u16SpeedLoopCount >= SpeedLoopPrescaler )  //速度环和位置环2KHZ频率执行
        {
            ...
            if ( psCmd->eCmdMode == CmdMode_Idle )  //标定状态
            {
                ...
                {
                    pi_spd.Ref = _IQdiv8 ( pi_pos.Umax );
                    pi_spd.Umax = _IQdiv ( ParamHandle_Steer.Reg_CurrentMax, 100 );
                    pi_spd.Umin = -pi_spd.Umax;
            ...
            else if ( ( psCmd->eCmdMode == CmdMode_TorqueCtrl ) && ( sTeles_Ctrl.PosCali_OK ) )
            {
                pi_spd.Ref = _IQdiv8 ( pi_pos.Umin );
                
                pi_spd.Umax = _IQdiv ( ParamHandle_Steer.Reg_CurrentMax * psCmd->iqTargetTorque / 100, 100 );
                pi_spd.Umin = -pi_spd.Umax;
            ...
            else if ( ( psCmd->eCmdMode == CmdMode_PosCtrl ) && ( sTeles_Ctrl.PosCali_OK ) )
            {
                ...
                {
                    pi_spd.Ref = _IQdiv4 ( pi_pos.Umax );
                ...
                else if ( sTeles_Ctrl.PosCmd_Dir == CtrlDir_Reverse )  //下降
                {
            ...
            else if ( psCmd->eCmdMode == CmdMode_RC )
            {
                if ( sTeles_Ctrl.PosCali_OK == 0 )
                {
                    psCmd->iqTargetSpd = _IQdiv8 ( pi_pos.Umax );
...
//        TXBuf[4] = _IQtoIQ12 ( pi_iq.Ref )>> 8;
//        TXBuf[5] = _IQtoIQ12 ( pi_iq.Ref );
//        TXBuf[6] = _IQtoIQ12 ( pi_iq.Fbk ) >> 8;
//        TXBuf[7] = _IQtoIQ12 ( pi_iq.Fbk );
...
//        TXBuf[7] = _IQtoIQ10 ((abs (pi_iq.Fbk)));
        ...
        TXBuf[0] = _IQtoIQ12 (psInfo->iqCurrentA) >>8;
        TXBuf[1] = _IQtoIQ12 (psInfo->iqCurrentA) ;
        TXBuf[2] = _IQtoIQ12 (psInfo->iqEffectCurrent) >>8;
        TXBuf[3] = _IQtoIQ12 (psInfo->iqEffectCurrent) ;
        TXBuf[4] = _IQtoIQ12 ( pi_iq.Ref )>> 8;
        TXBuf[5] = _IQtoIQ12 ( pi_iq.Ref );
        TXBuf[6] = _IQtoIQ12 ( pi_iq.Fbk ) >> 8;
        TXBuf[7] = _IQtoIQ12 ( pi_iq.Fbk );
...
static int BlockJudge ( void )
{
    ...
    if ( g_sMotor_Info.iqEffectCurrent > _IQ ( 1.5 ) )
    {
        if ( u16Cnt_Block < 2000 ) u16Cnt_Block++;
        else
        {
            u16Cnt_Block = 0;
            ret = 1;
