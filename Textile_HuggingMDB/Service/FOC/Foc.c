...
void FOC_PID_Init ( void )
{
    ...
    pi_spd.Kp = ParamHandle_Steer.Reg_PI_SPD_KP;  //速度环20ms
    ...
    pi_spd.Umax = _IQdiv ( 600, 100 );//ParamHandle_Steer.Reg_CurrentMax
...
//        psInfo->iqEffectCurrent = _IQdiv ( ( abs ( psInfo->iqCurrentA ) + abs ( psInfo->iqCurrentB ) + abs ( psInfo->iqCurrentC ) ), _IQ ( 1.732 ) );
        ...
        if ( ++u16SpeedLoopCount >= SpeedLoopPrescaler )  //速度环和位置环2KHZ频率执行
        {
            ...
            if ( psCmd->eCmdMode == CmdMode_Idle )  //标定状态
            {
                ...
                {
                    pi_spd.Ref = _IQdiv4 ( pi_pos.Umin );
            ...
            else if ( ( psCmd->eCmdMode == CmdMode_PosCtrl ) && ( sTeles_Ctrl.PosCali_OK ) )  //上位机位置控制
            {
                if ( sTeles_Ctrl.PosCmd_Dir == CtrlDir_Foward )  //缩回 
                {
                    pi_spd.Ref = _IQdiv2 ( pi_pos.Umin );
//                    if ( ( BlockJudge ( ) ) || ( sTeles_Ctrl.Rounds <= psCmd->iqTargetPos ) )
                    if ( psInfo->iqPosAbs <= ParamHandle_Steer.Reg_PosMin )
                    {
                        psCmd->eRunState = EndOfCtrl;
                    }
                    else if ( BlockJudge ( ) )
                ...
                {
                    pi_spd.Ref = _IQdiv2 ( pi_pos.Umax );
//                    if ( ( BlockJudge ( ) ) || ( sTeles_Ctrl.Rounds >= psCmd->iqTargetPos ) )
                    if ( psInfo->iqPosAbs >= ParamHandle_Steer.Reg_PosMax )
                    {
                        psCmd->eRunState = EndOfCtrl;
                    }
//                    else if ( BlockJudge ( ) )  //伸出越多，电流越大
//                    {
//                        psCmd->eRunState = CmdStop_PosLimit;
//                    }
                    else
            ...
            else if ( psCmd->eCmdMode == CmdMode_RC )  //遥控
            {
                if ( sTeles_Ctrl.PosCali_OK == 0 )
                {
                    psCmd->iqTargetSpd = _IQdiv4 ( pi_pos.Umin );
                }
                pi_spd.Ref = _IQsat ( psCmd->iqTargetSpd, pi_pos.Umax, pi_pos.Umin );
                if ( psCmd->iqTargetSpd < 0 )  //缩回
                {
                    ...
                    {
                        if ( ( BlockJudge ( ) ) || ( psInfo->iqPosAbs <= ParamHandle_Steer.Reg_PosMin ) )
                        {
                            pi_spd.Ref = 0;
                            psCmd->eRunState = EndOfCtrl;
                        }
                ...
                {
                    if ( ( BlockJudge ( ) ) || ( psInfo->iqPosAbs >= ParamHandle_Steer.Reg_PosMax ) )
                    {
                        pi_spd.Ref = 0;
                        psCmd->eRunState = EndOfCtrl;
                    }
...
//        TXBuf[0] = ( Clark.As >> 8 )>> 8;
//        TXBuf[1] = ( Clark.As >> 8 );
//        TXBuf[2] = ( Clark.Bs >> 8 ) >> 8;
//        TXBuf[3] = ( Clark.Bs >> 8 );
//        TXBuf[4] = ( Clark.Alpha >> 8 )>> 8;
//        TXBuf[5] = ( Clark.Alpha >> 8 );
//        TXBuf[6] = ( Clark.Beta >> 8 ) >> 8;
//        TXBuf[7] = ( Clark.Beta >> 8 );

//        TXBuf[0] = ( Park.Alpha >> 8 )>> 8;
//        TXBuf[1] = ( Park.Alpha >> 8 );
//        TXBuf[2] = ( Park.Beta >> 8 ) >> 8;
//        TXBuf[3] = ( Park.Beta >> 8 );
//        TXBuf[4] = ( Park.Sine >> 8 )>> 8;
//        TXBuf[5] = ( Park.Sine >> 8 );
//        TXBuf[6] = ( Park.Cosine >> 8 ) >> 8;
//        TXBuf[7] = ( Park.Cosine >> 8 );
...
//        TXBuf[6] = _IQtoIQ10 ((abs (pi_iq.Fbk))) >>8;;
//        TXBuf[7] = _IQtoIQ10 ((abs (pi_iq.Fbk)));
//        TXBuf[0] = _IQtoIQ10 ( pi_pos.Ref ) >> 8;
//        TXBuf[1] = _IQtoIQ10 ( pi_pos.Ref );
//        TXBuf[2] = _IQtoIQ10 ( pi_pos.Fbk ) >> 8;
//        TXBuf[3] = _IQtoIQ10 ( pi_pos.Fbk );
...
static int BlockJudge ( void )
{
    ...
    if ( g_sMotor_Info.iqEffectCurrent > _IQ ( 3 ) )
    {
        if ( u16Cnt_Block < 500 ) u16Cnt_Block++;
        else
        {
            u16Cnt_Block = 0;
            ret = 1;
