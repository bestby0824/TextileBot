...
//input
    ...
    for ( i = DI_CH_VID0; i < DI_CH_NUM; i++ )
    {
        GPIO_InitStruct.Pin = g_u16InputGPIOPin[i];
        HAL_GPIO_Init ( ( GPIO_TypeDef* ) g_psInputGPIOPort[i], &GPIO_InitStruct );
    }
...
void DidoLED_Init ( void )
{
    ...
    GPIO_InitStruct.Pin       = DO_LED_GPIO_PIN;
    HAL_GPIO_Init ( ( GPIO_TypeDef* ) DO_LED_GPIO_PORT, &GPIO_InitStruct );
    HAL_GPIO_WritePin ( ( GPIO_TypeDef* ) DO_LED_GPIO_PORT, DO_LED_GPIO_PIN, GPIO_PIN_RESET );
...
uint8_t DO_ReadAll ( uint16_t* pu16Value )
{
    ...
    {
        eStatus = HAL_GPIO_ReadPin ( ( GPIO_TypeDef* ) g_psOutputGPIOPort[i], g_u16OutputGPIOPin[i] );
