...
/*! \fn				void RelayCtrl_Send(ModbusTypeDef *pModbusVars)
...
void RelayCtrl_Send ( ModbusTypeDef *pModbusVars )
{
    ...
    u8Data[0] = pModbusVars->Id;
    u8Data[1] = pModbusVars->Featurecodes;
    u8Data[2] = ( ( pModbusVars->RegAddr >> 8 ) & 0x00FF );
    u8Data[3] = ( pModbusVars->RegAddr & 0x00FF );
    u8Data[4] = ( ( pModbusVars->RegData >> 8 ) & 0x00FF );
    u8Data[5] = ( pModbusVars->RegData  & 0x00FF );
    ...
    if ( sr & ( 1 << 4 ) ) //总线空闲(接收完成)
    {
        ...
        if ( g_u32RxLen == TAMA_REPLY_LENTH )
        {
            ...
            if ( u16Crc == Modbus_GetCRC16 ( g_pu8RxBuf, TAMA_REPLY_LENTH - 2 ) ) //CRC校验待添加
            {
                ...
                if ( g_pu8RxBuf[1] == RelayCtrl_Read )
                {
                    gRelayCmd.eFbMode = ( g_pu8RxBuf[3] << 8 ) + g_pu8RxBuf[4];
                }
                s16Relay1Life = FullLife_2s;
            }
        }
