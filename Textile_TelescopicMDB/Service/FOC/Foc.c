...
//-------------------- private functions ------------------------------------
static int BlockJudge ( void );
static _iq RunRoundsClc ( S_TELESCOPIC_CTRL sTeles_Ctrl_Handle );
static _iq ramper ( _iq in, _iq out, _iq rampDelta );
...
void FOC_PID_Init ( void )
{
    ...
    pi_spd.Kp = ParamHandle_Steer.Reg_PI_SPD_KP*3;  //速度环20ms
    ...
    pi_spd.Umax = _IQdiv ( ParamHandle_Steer.Reg_CurrentMax, 100 );//_IQdiv ( 2100, 100 );1\4号车电源模块500W，电流限制在21A
...
_iq iqtest_en = 0, iqtest_value = 0, iqtest_cnt = 0;
void FOC_Core ( S_MOTOR_INFO *psInfo, S_MOTOR_CMD *psCmd )
{
    static uint16_t u16SpeedLoopCount = 0;           // Speed loop counter
    static uint16_t u16SpdAccCount = 0;

    if ( ( ( psCmd->eRunState == CmdRuning ) ) && ( psInfo->u16MotorVol_State ) )  //动力电&控制模式
    {
        ...
        if ( ++u16SpeedLoopCount >= SpeedLoopPrescaler )  //速度环和位置环2KHZ频率执行
        {
            ...
            if ( psCmd->eCmdMode == CmdMode_Idle )  //标定状态
            {
                if ( sTeles_Ctrl.PosCali_OK == 0 )
                {
                    pi_spd.Ref = _IQdiv16 ( pi_pos.Umin );  //缩回
                    
                    if ( BlockJudge ( ) )
                    {
                        sTeles_Ctrl.PosMinCali = 1;
                        sTeles_Ctrl.PosCali_OK = 1;
            ...
            else if ( ( psCmd->eCmdMode == CmdMode_PosCtrl ) && ( sTeles_Ctrl.PosCali_OK ) )
            {
                pi_pos.Ref = psCmd->iqTargetPos;
                pi_pos.Fbk = psInfo->iqPosAbs;
                Pos_Loop ( &pi_pos );
                
                if ( sTeles_Ctrl.PosCmd_Dir == CtrlDir_Foward )
                {
                    if ( ++u16SpdAccCount > 20 )  //100Hz
                    {
                        u16SpdAccCount = 0;
//                        if ( abs ( pi_pos.up ) < _IQ ( 0.1 ) )
//                        {
//                            psCmd->iqTargetSpd = _IQdiv64 ( pi_pos.Umax );
//                        }
//                        else
//                        {
//                            psCmd->iqTargetSpd = _IQdiv32 ( pi_pos.Umax );
//                        }
                        psCmd->iqTargetSpd = _IQdiv32 ( pi_pos.Umax );
                        pi_spd.Ref = ramper ( psCmd->iqTargetSpd, pi_spd.Ref, ParamHandle_Steer.Reg_AccSpd );
                    }
                    
                    if ( pi_pos.up < ParamHandle_Steer.Reg_PosDeadZone )
                    {
                ...
                else if ( sTeles_Ctrl.PosCmd_Dir == CtrlDir_Reverse )
                {
                    if ( ++u16SpdAccCount > 20 )  //100Hz
                    {
                        u16SpdAccCount = 0;
//                        if ( abs ( pi_pos.up ) < _IQ ( 0.1 ) )
//                        {
                            psCmd->iqTargetSpd = _IQdiv32 ( pi_pos.Umin );
//                        }
//                        else
//                        {
//                            psCmd->iqTargetSpd = _IQdiv8 ( pi_pos.Umin );
//                        }
                        pi_spd.Ref = ramper ( _IQsat ( psCmd->iqTargetSpd, pi_pos.Umax, pi_pos.Umin ), pi_spd.Ref, ParamHandle_Steer.Reg_AccSpd );
                    }
                    ...
                    else if ( BlockJudge ( ) )
                    {
                        u16Cnt_Posctrl = 0;
                        pi_spd.Ref = 0;
                        psCmd->eRunState = CmdStop_PosLimit;
                    }
                    else
                    {
                        u16Cnt_Posctrl = 0;    
                ...
                else if ( sTeles_Ctrl.PosCmd_Dir == CtrlDir_ExtenHold )
                {
                    if ( ++u16SpdAccCount > 20 )  //100Hz
                    {
                        u16SpdAccCount = 0;
                    ...
                    if ( ( psInfo->u16Switch_State ) || ( psInfo->iqPosAbs >= ParamHandle_Steer.Reg_PosMax ) )
                    {
                        pi_spd.Ref = 0;
                        if ( psInfo->u16Switch_State )
                        {
                            psCmd->eRunState = CmdStop_Switch;
                        }
                        else
                        {
                            psCmd->eRunState = EndOfCtrl;
                ...
                {
                    pi_spd.Ref = 0;
                    psCmd->eRunState = CmdStop_Invalid;
                }
            }
            else if ( psCmd->eCmdMode == CmdMode_RC )
            {
                pi_spd.Ref = psCmd->iqTargetSpd;
                
                if ( psCmd->iqTargetSpd < 0 )  //缩回  
                {
                    if ( BlockJudge ( ) )
                    {
                        sTeles_Ctrl.PosMinCali = 1;
                ...
                {
                    if ( ( psInfo->u16Switch_State ) || ( psInfo->iqPosAbs >= ParamHandle_Steer.Reg_PosMax ) )  //5圈1cm,55cm
                    {
                        psCmd->eRunState = EndOfCtrl; 
                    }
                }
            ...
            {
                pi_spd.Ref = 0;
                psCmd->eRunState = CmdStop_NoneCali;
            }
...
//        if ( psCmd->eCmdMode == CmdMode_TorqueCtrl )
//        {
//            pi_iq.Ref = _IQsat ( _IQ16 ( _IQrmpy ( psCmd->iqTargetTorque, _IQ ( ParamHandle_Steer.Reg_ReverseFlag ) ) ), _IQ(10), -_IQ(10) );
//        }
//        else
//        {
            pi_iq.Ref = _IQrmpy ( pi_spd.Out, _IQ ( ParamHandle_Steer.Reg_ReverseFlag ) );
//        pi_iq.Ref = iqtest_value;
//        }
...
//        TXBuf[0] = _IQtoIQ12 ( pi_pos.Ref )>> 8;
//        TXBuf[1] = _IQtoIQ12 ( pi_pos.Ref );
//        TXBuf[2] = _IQtoIQ12 ( pi_pos.Fbk ) >> 8;
//        TXBuf[3] = _IQtoIQ12 ( pi_pos.Fbk );
        TXBuf[0] = _IQtoIQ10 ( psInfo->iqEffectCurrent )>> 8;
        TXBuf[1] = _IQtoIQ10 ( psInfo->iqEffectCurrent );
        TXBuf[2] = _IQtoIQ12 ( pi_pos.Fbk ) >> 8;
        TXBuf[3] = _IQtoIQ12 ( pi_pos.Fbk );
        TXBuf[4] = _IQtoIQ12 ( pi_spd.Ref ) >> 8;
        TXBuf[5] = _IQtoIQ12 ( pi_spd.Ref );
        TXBuf[6] = _IQtoIQ12 ( pi_spd.Fbk ) >> 8;
        TXBuf[7] = _IQtoIQ12 ( pi_spd.Fbk );
        
//        TXBuf[4] = _IQtoIQ12 ( psInfo->iqPosAdd )>> 8;
//        TXBuf[5] = _IQtoIQ12 ( psInfo->iqPosAdd );
//        TXBuf[6] = _IQtoIQ12 ( psInfo->iqEAngle ) >> 8;
//        TXBuf[7] = _IQtoIQ12 ( psInfo->iqEAngle );
...
//        TXBuf[0] = TrigFunc_Handle.Angle_Coder_Current >> 8;
//        TXBuf[1] = TrigFunc_Handle.Angle_Coder_Current;
//        TXBuf[2] = _IQtoIQ8 (TrigFunc_Handle.Angle_Coder_Total) >> 8;
//        TXBuf[3] = _IQtoIQ8 (TrigFunc_Handle.Angle_Coder_Total);
//        TXBuf[4] = TrigFunc_Handle.Angle_Current >> 8;
//        TXBuf[5] = TrigFunc_Handle.Angle_Current;
//        TXBuf[6] = TrigFunc_Handle.Length_Pos >> 8;
//        TXBuf[7] = TrigFunc_Handle.Length_Pos;
...
{
    int ret = 0;
    static uint16_t u16Cnt_Block = 0;
    
    if ( g_sMotor_Info.iqEffectCurrent > _IQ ( 10 ) )
    {
        if ( u16Cnt_Block < 3000 ) u16Cnt_Block++;
        else
        {
            u16Cnt_Block = 0;
            ret = 1;
