...
static int RelayLackCheck ( void );
...
static void SetPosCoderInvalid_Warn ( void );
...
int16_t s16Relay1Life = FullLife_2s;
...
{
    FaultCheckEnable = 0x0001FCF;  //上电自检
    WarnCheckEnable = 0x000001FF;
    /************************电机出轴编码器离线检测****************************/
...
//    }
    ...
    if ( sTeles_Ctrl.PosCali_OK ) {
        g_u32WarningNum.bit.uPosCoderInvalid = 0;
    } else {
        SetPosCoderInvalid_Warn ();
    }
    /************************继电器模块离线检测****************************/
    if ( ( WarnCheckEnable & BIT ( 8 ) ) && ( u8Standby_Flag ) )
    {
        if ( 0 == g_u32FaultNum.bit.uCoderTama2_Lack )
        {
            g_u32WarningNum.bit.uRelayLinkLose = RelayLackCheck ( );
        }
        else
        {
            ;
    ...
    {
        g_u32WarningNum.bit.uRelayLinkLose = 0;
    }
}
/**
 * @brief       电机出轴编码器离线检测
...
static int RelayLackCheck ( void )
{
    ...
    if ( s16Relay1Life > 0 ) s16Relay1Life--;
    if ( s16Relay1Life == 0 )
    {
        res = Fault;
    }
...
static int LodeOverCheck ( void )
{
    ...
    _iqIqLoseLim = _IQdiv ( ParamHandle_Steer.Reg_RatedCurrent * 1.5, 100 );
...
static void SetPosCoderInvalid_Warn ( void )
{
    if ( ( WarnCheckEnable & BIT ( 7 ) ) )
    {
        g_u32WarningNum.bit.uPosCoderInvalid = Fault;
    }
}
/**
 * @brief       温度检测函数
