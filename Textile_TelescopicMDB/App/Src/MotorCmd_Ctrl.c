...
void MotorCmd_Ctrl ( RegDef_PosCtrlCmd *PosCtrlCmd, S_MOTOR_CMD *sMotorCmd )
{
    ...
    static RegDef_PosCtrlCmd Rx_SteerCtrlCmd = { CmdMode_FreeStop, 0, 0, _IQ ( 0.25 ) };
    ...
    case CmdMode_PosCtrl:
    {
        ...
        if ( ( u16SteerCmdLastSN != Rx_SteerCtrlCmd.u16SN ) || ( eLastCmdMode != CmdMode_PosCtrl ) )
        {
            ...
            else
            {
                ...
                {
                    if ( Rx_SteerCtrlCmd.u16Ref == EXTENHOLDCMD )
                    {
                        sTeles_Ctrl.PosCmd_Dir = CtrlDir_ExtenHold;
                        sMotorCmd->eRunState = CmdRuning;
                    ...
                    else
                    {
                        ...
                        if ( ( sMotorCmd->iqTargetPos - g_sMotor_Info.iqPosAbs ) > ParamHandle_Steer.Reg_CtrlDeadZone ) //2mm
                        {
                            sTeles_Ctrl.PosCmd_Dir = CtrlDir_Foward; //伸出
                            sMotorCmd->eRunState = CmdRuning;
                            sMotorCmd->iqRuningTime = 0;
                        }
                        else if ( ( sMotorCmd->iqTargetPos - g_sMotor_Info.iqPosAbs ) < -ParamHandle_Steer.Reg_CtrlDeadZone )
                        {
    ...
    case CmdMode_RC:
    {
        ...
        {
            sMotorCmd->eRunState = CmdRuning;
            sMotorCmd->iqTargetSpd = ParamHandle_Steer.Reg_RC_Spdmax/4;
        }
