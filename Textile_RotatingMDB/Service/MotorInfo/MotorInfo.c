...
void Encoder_Init( Encoder_Struct *encoder ) {
    static uint16_t u16EncoderData[10], u16EncoderMax = 0, u16EncoderMin = 0xFFFF;
    static _iq iqEncoderSum;
    
//    for ( int i = 0; i < 10; i++ )
//    {
//        u16EncoderData[i] = GetReangleValue ( );
//        if ( u16EncoderData[i] > u16EncoderMax ) u16EncoderMax = u16EncoderData[i];
//        if ( u16EncoderData[i] < u16EncoderMin ) u16EncoderMin = u16EncoderData[i];
//        iqEncoderSum += u16EncoderData[i];
//        HAL_Delay ( 1 );
//    }

//    encoder->current_initpos = iqEncoderSum - u16EncoderMax - u16EncoderMin ;
    encoder->current_position = 0;
    encoder->last_position = 0;
    encoder->total_ticks = 0;
    encoder->revolution_count = 0;
    encoder->linear_distance = 0;
}
...
void Encoder_PosClc( Encoder_Struct *encoder, uint8_t u8Enable_Clc ) {
    ...
    // 计算线性距离 (米)
    encoder->linear_distance = encoder->total_ticks / 200 + _IQ ( 0.5 ); 
//    encoder->linear_distance = _IQdiv ( encoder->total_ticks, ( ParamHandle_Steer.Reg_PolePairs * 400 ) ) + _IQ ( 0.5 );
    } else {
       encoder->current_position = GetReangleValue ( ); 
    }
}
